...
##################Registration No. ##################
# it is created to automatic enter registration_no():
...
def Save():
    ...
    if N1 == "" or C1 == "Select Class" or D2 == "" or Re1 == "" or S1 == "" or fathername == "" or mothername == "" or F1 == "" or M1 == "":
        ...
        try:
            ...
            Clear()  # clear entry box image section
            registration_no()  # it will recheck registration no.and reissue new no


###############Search############################
def search():
    text = Search.get()     #taking input from entry boy
...
# top frames
Label(root, text="Email: emergenceacademique@gmail.com", width=10, height=3, bg="#f0687c", anchor='e').pack(side=TOP,
                                                                                                            fill=X)
...
Search = StringVar()
Entry(root, textvariable=Search, width=12, bd=2, font="arial 20").place(x=820, y=70)
...
# Student details
obj = LabelFrame(root, text="Student's Detail", font=20, bd=2, width=900, bg=framebg, fg=framefg, height=250,
                 relief=GROOVE)
...
Button(root, text="Exist", width=19, height=2, font="arial 12 bold", bg="green", command=Exit).place(x=1000, y=610)
